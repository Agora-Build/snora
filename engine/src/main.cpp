...
int main(int argc, char *argv[]) {
  ...
  if (!got_init) {
    fprintf(stderr, "{\"level\":\"error\",\"msg\":\"Did not receive init "
  ...
  const auto frame_interval =
      std::chrono::milliseconds(snora::FRAME_DURATION_MS);
    ...
    for (const auto &msg : messages) {
      if (msg.type == "state_update") {
        ...
        physio.respiration_rate =
            msg.data.value("respiration_rate", physio.respiration_rate);
        physio.stress_level =
