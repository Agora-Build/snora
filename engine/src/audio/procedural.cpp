...
void ProceduralTexture::process(int16_t *buffer, int num_samples,
                                float intensity) {
  switch (type_) {
  case Type::Rain:
    process_rain(buffer, num_samples, intensity);
    break;
  case Type::Wind:
    process_wind(buffer, num_samples, intensity);
    break;
  case Type::Ocean:
    process_ocean(buffer, num_samples, intensity);
    break;
  }
}
...
void ProceduralTexture::process_rain(int16_t *buffer, int num_samples,
                                     float intensity) {
  if (intensity <= 0.0f)
    return;
  ...
  for (int i = 0; i < frames; ++i) {
    for (int ch = 0; ch < CHANNELS; ++ch) {
      ...
      if (g.samples_remaining > 0) {
        ...
        // One-pole low-pass to smooth the burst (simulates a raindrop's
        // spectral shape)
        g.filter_state = LP_COEFF * g.filter_state + (1.0f - LP_COEFF) * white;

        // Hanning window for grain envelope (avoid clicks)
        float t = 1.0f - static_cast<float>(g.samples_remaining) /
                             static_cast<float>(g.grain_length);
        float window =
      ...
      buffer[idx] =
          static_cast<int16_t>(std::max(-32767.0f, std::min(32767.0f, mixed)));
    }
  }
}
...
void ProceduralTexture::process_wind(int16_t *buffer, int num_samples,
                                     float intensity) {
  if (intensity <= 0.0f)
    return;
  ...
  for (int i = 0; i < frames; ++i) {
    ...
    float omega = 2.0f * static_cast<float>(M_PI) * center_hz /
    ...
    for (int ch = 0; ch < CHANNELS; ++ch) {
      ...
      // One-pole LP
      filter_state_[ch] =
          lp_coeff * filter_state_[ch] + (1.0f - lp_coeff) * white;
...
// ─────────────────────────────────────────────────────────────────────
//
// Brown (red) noise with slow amplitude modulation at wave rhythm rate
// (0.05-0.15 Hz). Brown noise is produced by integrating white noise with a
// leaky integrator.

void ProceduralTexture::process_ocean(int16_t *buffer, int num_samples,
                                      float intensity) {
  if (intensity <= 0.0f)
    return;
  ...
  constexpr double WAVE_LFO_FREQ =
      0.01; // Hz — very slow modulation of wave rate
  ...
  for (int i = 0; i < frames; ++i) {
    ...
    // Use a second phase accumulator tracked within the LFO phase for
    // simplicity: We embed wave phase in phase_ by letting it tick at
    // wave_rate.
